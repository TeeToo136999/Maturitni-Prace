...
function Fleet(name) {
	this.name = name;
	this.shipDetails = [{ "name": "letadlová loď", "length": 5 },
						{ "name": "bitevní loď", "length": 4 },
						{ "name": "ponorka", "length": 3 },
						{ "name": "křižník", "length": 3 },
						{ "name": "torpédoborec", "length": 2 }];
...
var output = {
	...
	lost: function(name) { return " > " + name + " prohrál!!  Konec hry." },
};
...
function gameSetup(t) {
	$(t).off() && $(".two").off();
	$(".one").addClass("self").removeClass("one").text("Sám");
	$(".multi").addClass("random").removeClass("multi").text("Náhodně");
...
function selfSetup() {
	$(".self").addClass("horz").removeClass("self").text("Horizontální");
	$(".random").addClass("vert").removeClass("random").text("Vertikální");
...
function setShip(location, ship, orientation, genericFleet, type) {
	if (!(checkOverlap(location, ship.length, orientation, genericFleet))) {
		if (orientation == "horz") {
			...
			$(".text").text(output.placed(genericFleet.ships[genericFleet.currentShip].name + " byla"));
			for (var i = location; i < (location + ship.length); i++) {
			...
			if (++genericFleet.currentShip == genericFleet.numOfShips) {
				$(".text").text(output.placed("lodě byli"));
				$(".bottom").find(".points").off("mouseenter");
				setTimeout(createCpuFleet, 100);
			} else {
				if (type == "random") randomSetup(genericFleet);
